...
#define TXPIN 12
#define BMPPIN 0x77                 // Sensor address
#define DHTPIN 2        // Digital pin the DHT22 is connected to
#define DHTTYPE DHT22   // Sensor type (DHT11, DHT22 etc)
...
void loop()
{
 ...
 float h = dht.readHumidity();         // Read humidity
  float t = dht.readTemperature();      // Read temperatre in celcius
  float f = dht.readTemperature(true);  // Read temperature in farenheit
 ...
 SendData("testing");
 delay(2000);                               // Delay between each readout
...
int32_t temperature()
{
 ...
 Wire.beginTransmission(BMPPIN); // Start transmission to device 
...
int32_t read_pressure()
{
 ...
 Wire.requestFrom(BMPPIN, 3);      // Request three bytes
 if(Wire.available() >= 3)
 {
...
uint8_t read_1_byte(uint8_t code)
{
 ...
 Wire.endTransmission();                         // End transmission
 Wire.requestFrom(BMPPIN, 1);            // Request data for 1 byte to be read
 if(Wire.available() >= 1)
 {
...
uint16_t read_2_bytes(uint8_t code)
{
 ...
 Wire.write(code);                               // Sends register address to read from
 Wire.endTransmission();                         // End transmission
 Wire.requestFrom(BMPPIN, 2);            // Request 2 bytes from device
 if(Wire.available() >= 2)
 {
